...
class Engine(object):
#the main "start" of the games engine keeps the game running straight
#in this case. it takes a variable that has an instance of the Map() as a param

	def __init__(self, scene_map):
		#self.scene_map is variable scene_map
		self.scene_map = scene_map

	def play(self):

	#set current_scene as self.scene_map using opening_scene()
	#opening_scene() is in map.opening_scene().....
...
class CentralCorridor(Scene):
	...
	def enter(self):
		print(dedent("""
			The Gothons of Planet Percal #25 have invaded your ship and destroyed your entire crew. You are the last surviving member and your alst mission is to get the neutron bomb from the weapons Armory, put it in the bridge, and blow the ship up after getting into an escape pod. You're runnig downt he central corridor to the Weapons Armory when a Gothon jumps out, red scaly skin, dark grimy teeth, and evil clown costume flowing around his hate filled body. He's blocking the door to the armory and about to pull a weapon to blast you.
			"""))
		action = input("> ")
		if action == "shoot!":
			print(dedent(""" 
				Quick on the draw you yank out your blaster and fire it at the Gothon. His clown costume is flowing and mvoing around his body, which throws off your aim. Your laser hits his costume but misses him entirely. This completely ruins his brand new costume his mother bought him, which makes him fly into an insane rage and blast you repeatedly in the face until you are dead. Then he eats you.
				"""))
		elif action == "dodge!":
			print(dedent("""
				Like a world class boxer you dodge, weave, slip and slide right as the Gothon's blaster cranks a laser past your head. In the middle of your artful dodge your foot slips and you bang your head on the metal wall and pass out. You wake up shortyly after only to die as the Gothon stomps on your head and eats you. 
				"""))
			return 'death'
		elif action == "tell a joke":
			print(dedent("""
				Lucky for you they made you learn Gothon insults in the academy. You tell the one Gothon joke you know: Lbhe zbgure vf fb sng, jra fur fbgf nebhaq gur ubhfr, fur fvgf nebhaq gur ubhfr. The Gothon stops, tries not to laugh , then busts out laughing and can't move. While he's laughing you run up and shoot him qiuare in the head putting him down, then jump through the Weapon Armory door.
				"""))
...
class LaserWeaponArmory(Scene):
	...
	def enter(self):
		...
		if guess == code:
			print(dedent("""
				The container clicks open and the seal breaks, letting gas out. You grab the neutron bomb and run as fas you can to the bridge where you must place it in the right spot.
			"""))
			return 'the_bridge'
		else:
			print(dedent(""" 
...
class TheBridge(Scene):
	...
	def enter(self):
		...
		if action == "throw the bomb":
			...
			print(dedent("""
				You point your blaster at the bomb under your arm and the Gothons put their hands up and start to seat. You inch backward to the door, open it, and then carefully place the bomb on the floor, pointing your blaster at it. You then jump back through the door, punch the close button and blast the lock so the Gothons can't get out. Now that the bomb is place you run to the escape pod to get off this tin can.
				"""))
			return 'escape_pod'
		else:
			print("DOES NOT COMPUTE!")
...
class Map(object):
	...
	#fucking get. Get makes it so any time a return with a string runs it checks it with the 'scenes' dict and runs the corresponding value
	...
	def next_scene(self, scene_name):
		...
		#i understand how map.opening_scene() gets pulled to the very top of the code('calling')

	def opening_scene(self):

			#return next_scene and start next scene
...
a_map = Map('central_corridor')
	...
	# set a_game to Engine, Engine will take a_map as a parameter and excute Map() on call
	#mad

a_game = Engine(a_map)

#call .play from class Engine()
